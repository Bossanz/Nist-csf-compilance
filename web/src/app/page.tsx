...
export default function Home() {
  ...
  async function deleteProject(item:Project){setOpeningID(item.id);setError("");try{await api.deleteProject(item.id);setProjects(items=>items.filter(project=>project.id!==item.id))}catch(e){setError(e instanceof Error?e.message:"Could not delete project")}finally{setOpeningID("")}}
  ...
  if(!project)return <ProjectDashboard projects={projects} loading={loadingProjects} openingID={openingID} error={error} onOpen={openProject} onCreate={createProject} onDelete={deleteProject}/>;
