...
export default function Home() { const [functions,setFunctions]=useState<FunctionNode[]>([]); const [project,setProject]=useState<Project|null>(null); const [profile,setProfile]=useState<ProfileRow[]>([]); const [summary,setSummary]=useState<Summary>({coveragePct:0,includedCount:0,pendingCount:0,rejectedCount:0,functions:[]}); const [selected,setSelected]=useState(""); const [name,setName]=useState(""); const [organizationName,setOrganizationName]=useState(""); const [error,setError]=useState("");
  ...
  async function save(id:string,patch:Parameters<typeof api.updateProfile>[2]){if(!project)return;await api.updateProfile(project.id,id,patch);setSummary(await api.getSummary(project.id));}
  ...
  return <div className="shell"><FunctionSidebar functions={functions} selectedCode={selected} onSelect={setSelected}/><main className="main"><div className="eyebrow">Project / {project.status}</div><h1>{project.name}</h1><p className="muted">Profile workspace</p><SummaryCards summary={summary}/><div className="panel"><h2>{selected} profile</h2><p className="muted">Set current and target coverage for each outcome.</p><ProfileEditor rows={visible} onSave={save}/></div></main></div>;
}
