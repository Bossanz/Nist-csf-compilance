...
export const api = {
  ...
  requestPasswordReset: (email: string) => request<{ message: string }>("/api/auth/password-reset/request", { method: "POST", body: JSON.stringify({ email }) }),
  confirmPasswordReset: (token: string, password: string) => request<void>("/api/auth/password-reset/confirm", { method: "POST", body: JSON.stringify({ token, password }) }),
  ...
  getOrganizationInvitations: (id:string) => request<Invitation[]>(`/api/organizations/${id}/invitations`),
  ...
  createInvitation: (organizationID:string,input:{email:string;role:Role;projectIDs?:string[]}) => request<Invitation>(`/api/organizations/${organizationID}/invitations`,{method:"POST",body:JSON.stringify(input)}),
  resendInvitation: (organizationID:string, invitationID:string) => request<Invitation>(`/api/organizations/${organizationID}/invitations/${invitationID}/resend`,{method:"POST",body:JSON.stringify({})}),
  cancelInvitation: (organizationID:string, invitationID:string) => request<Invitation>(`/api/organizations/${organizationID}/invitations/${invitationID}/cancel`,{method:"POST",body:JSON.stringify({})}),
  ...
  updateFunctionScope: (projectID: string, functionCode: string, included: boolean) => request<ProfileRow[]>(`/api/projects/${projectID}/functions/${encodeURIComponent(functionCode)}/scope`, { method: "PUT", body: JSON.stringify({ included }) }),
  ...
  getRemediationActions: (projectID: string) => request<RemediationAction[]>(`/api/projects/${projectID}/remediation-actions`),
  createRemediationAction: (projectID: string, input: RemediationCreateInput) => request<RemediationAction>(`/api/projects/${projectID}/remediation-actions`, { method: "POST", body: JSON.stringify(input) }),
  updateRemediationAction: (projectID: string, actionID: string, patch: RemediationPatchInput) => request<RemediationAction>(`/api/projects/${projectID}/remediation-actions/${actionID}`, { method: "PATCH", body: JSON.stringify(patch) }),
  updateRemediationProgress: (projectID: string, actionID: string, progressNote: string) => request<RemediationAction>(`/api/projects/${projectID}/remediation-actions/${actionID}/progress`, { method: "PATCH", body: JSON.stringify({ progressNote }) }),
  submitRemediationAction: (projectID: string, actionID: string) => request<RemediationAction>(`/api/projects/${projectID}/remediation-actions/${actionID}/submit`, { method: "POST", body: JSON.stringify({}) }),
  reviewRemediationAction: (projectID: string, actionID: string, input: { decision: "close" | "return"; comment: string }) => request<RemediationAction>(`/api/projects/${projectID}/remediation-actions/${actionID}/review`, { method: "POST", body: JSON.stringify(input) }),
  uploadRemediationEvidence: (projectID: string, actionID: string, file: File) => { const form = new FormData(); form.append("file", file); return request<RemediationEvidence>(`/api/projects/${projectID}/remediation-actions/${actionID}/evidence`, { method: "POST", body: form }); },
  downloadRemediationEvidence: (projectID: string, actionID: string, evidenceID: string, signal?: AbortSignal) => download(`/api/projects/${projectID}/remediation-actions/${actionID}/evidence/${evidenceID}`, signal),
  previewRemediationEvidence: (projectID: string, actionID: string, evidenceID: string, signal?: AbortSignal) => download(`/api/projects/${projectID}/remediation-actions/${actionID}/evidence/${evidenceID}/preview`, signal),
  deleteRemediationEvidence: (projectID: string, actionID: string, evidenceID: string) => request<void>(`/api/projects/${projectID}/remediation-actions/${actionID}/evidence/${evidenceID}`, { method: "DELETE" }),
