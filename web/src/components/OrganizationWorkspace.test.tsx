...
test("presents projects as a workspace register",()=>{render(<OrganizationWorkspace user={counselor} organization={organization} projects={[project]} users={[orgAdmin]} loading={false} error="" onBack={vi.fn()} onOpen={vi.fn()} onCreateProject={vi.fn()} onDeleteProject={vi.fn().mockResolvedValue(undefined)} onInvite={vi.fn()}/>);expect(screen.getByRole("region",{name:/project workspace register/i})).toBeTruthy();expect(screen.getByText("Readiness")).toBeTruthy();expect(screen.getByText("Setup")).toBeTruthy();expect(screen.getByRole("button",{name:/open readiness/i})).toBeTruthy()});
