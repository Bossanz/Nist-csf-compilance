...
func (s *Store) CreateProject(ctx context.Context, name, organizationName string) (Project, error) {
	tx, err := s.DB.Begin(ctx); if err != nil { return Project{}, err }; defer tx.Rollback(ctx)
	var orgID, projectID string
	if err = tx.QueryRow(ctx, `INSERT INTO organizations(name,type) VALUES ($1,'client') RETURNING id`, organizationName).Scan(&orgID); err != nil { return Project{}, err }
...
func (s *Store) ListProfile(ctx context.Context, projectID string) ([]ProfileRow, error) {
	if _, err := s.GetProject(ctx, projectID); err != nil { return nil, err }
	...
	for rows.Next() { var p ProfileRow; if err := rows.Scan(&p.ID,&p.ProjectID,&p.SubcategoryID,&p.FunctionCode,&p.CategoryCode,&p.SubcategoryCode,&p.Description,&p.Included,&p.Rationale,&p.CurrentPriority,&p.CurrentCoverageLevel,&p.CurrentStatusText,&p.CurrentPoliciesText,&p.CurrentTier,&p.TargetPriority,&p.TargetCoverageLevel,&p.TargetApproachText,&p.TargetTier,&p.Notes,&p.Considerations,&p.ReviewStatus); err != nil { return nil, err }; out = append(out,p) }
	return out, rows.Err()
}

func (s *Store) UpdateProfile(ctx context.Context, projectID, subcategoryID string, patch ProfilePatch) (ProfileRow, error) {
	if _, err := uuid.Parse(projectID); err != nil { return ProfileRow{}, fmt.Errorf("invalid project id") }
	if _, err := uuid.Parse(subcategoryID); err != nil { return ProfileRow{}, fmt.Errorf("invalid subcategory id") }
	sets, args := []string{}, []any{}; add := func(column string, value any) { args = append(args,value); sets = append(sets, fmt.Sprintf("%s=$%d",column,len(args))) }
	if patch.Included != nil { add("included",*patch.Included) }; if patch.Rationale != nil { add("rationale",*patch.Rationale) }; if patch.CurrentPriority != nil { add("current_priority",*patch.CurrentPriority) }; if patch.CurrentCoverageLevel != nil { add("current_coverage_level",*patch.CurrentCoverageLevel) }; if patch.CurrentStatusText != nil { add("current_status_text",*patch.CurrentStatusText) }; if patch.CurrentPoliciesText != nil { add("current_policies_text",*patch.CurrentPoliciesText) }; if patch.TargetPriority != nil { add("target_priority",*patch.TargetPriority) }; if patch.TargetCoverageLevel != nil { add("target_coverage_level",*patch.TargetCoverageLevel) }; if patch.TargetApproachText != nil { add("target_approach_text",*patch.TargetApproachText) }; if patch.Notes != nil { add("notes",*patch.Notes) }; if patch.Considerations != nil { add("considerations",*patch.Considerations) }
	if len(sets) == 0 { return ProfileRow{}, fmt.Errorf("no fields to update") }; args = append(args,projectID,subcategoryID)
	_, err := s.DB.Exec(ctx, `UPDATE project_subcategory_profiles SET `+strings.Join(sets,",")+` WHERE project_id=$`+fmt.Sprint(len(args)-1)+` AND subcategory_id=$`+fmt.Sprint(len(args)), args...); if err != nil { return ProfileRow{}, err }
	rows, err := s.ListProfile(ctx, projectID); if err != nil { return ProfileRow{}, err }; for _, row := range rows { if row.SubcategoryID == subcategoryID { return row,nil } }; return ProfileRow{}, fmt.Errorf("profile not found")
}
