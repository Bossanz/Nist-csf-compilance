...
func (h *Handler) profile(w http.ResponseWriter,r *http.Request,id string) { p,err:=h.Store.ListProfile(r.Context(),id); if errors.Is(err,pgx.ErrNoRows) { writeError(w,404,"not_found","project not found"); return }; if err!=nil { writeError(w,500,"internal_error","could not load profile"); return }; writeJSON(w,200,p) }
