...
$first = $profile[0]
$updated = Invoke-RestMethod -Method Put -Uri http://localhost:8080/api/projects/$($project.id)/profile/$($first.subcategoryID) -ContentType 'application/json' -Body (@{included=$true; currentCoverageLevel='partial'; targetCoverageLevel='full'} | ConvertTo-Json)
if (-not $updated.included -or $updated.currentCoverageLevel -ne 'partial') { throw 'profile update failed' }
